...
int main() {
    const int TREE_MEASURE = 2;
    TBTree* tree = new TBTree(TREE_MEASURE);
    TBTree* buffTree = new TBTree(TREE_MEASURE);
    char buff[256];
    TString key;
    ...
    while (std::cin >> buff) {
        ...
        if (buff[0] == '!') {
            ...
            if (stringCompare(buff, "Check")) {
                std::cout << "checking\n";
                tree->checkParentage();
                std::cout << "checked\n";
            } else
            if (stringCompare(buff, "Print")) {
                tree->printTree();
            } else {
