...
TBigInt TBigInt::operator+(const TBigInt &obj)
{
    TBigInt res;
    int carry = 0;
    for (unsigned int i = 0; i < std::max(mData.size(), obj.mData.size()) || carry; ++i) {
        int aa = i < (unsigned int) mData.size() ? mData[i] : 0;
        int bb = i < (unsigned int) obj.mData.size() ? obj.mData[i] : 0;
...
TBigInt TBigInt::operator-(const TBigInt &obj)
{
    TBigInt res;
    int carry = 0;
    for (unsigned int i = 0; i < mData.size() || carry; ++i) {
        int aa = i < (unsigned int) mData.size() ? mData[i] : 0;
        int bb = i < (unsigned int) obj.mData.size() ? obj.mData[i] : 0;
